...
print("------------------------------------------------------")
print("Appel KMeans pour une valeur de 2 a 10 clustersdonnées init)")
tps1 = time.time()

nombre_indice=list(range(2,15))
coeff_matrix=[]
best_coeff=0
for k in nombre_indice :
...
# Résultat du clustering
    ...
    coeff_matrix.append(silh)
    if best_coeff<silh:
        best_coeff=silh
    
print("------------------------------------------------------")
print("Comparaison des coeff pour chaque nombre de cluster")
plt.plot(nombre_indice,coeff_matrix, label="Coeff courb")
